...
class WorkerForm(QMainWindow):
    def __init__(self):
        ...
        self.workers.pushButton_back.clicked.connect(self.back_home)
    ...
    def addToDb(self):
        ...
        with sqlite3.connect('database_hrm.db') as dbconn:
            c = dbconn.cursor()
        c.execute(
            "INSERT INTO user_workers(National_ID, Registration_Date, KRA_PIN, First_Name, Middle_Name, Last_Name, Gender, Phone, Email, DOB, User_role,Experience_level,Skills)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (nid, reg_date, kra, fn, mn, ln, gender, phone, email, dob, urole, exp, skill))
    ...
    def delrow(self):
        ...
        if self.workers.tableWidget_records.currentIndex().row() > -1:
            self.model.removeRow(self.workers.tableWidget_records.currentIndex().row())
            self.i -= 1
            self.model.select()
            self.workers.lcdNumber_user_id.display(self.i)
        else:
